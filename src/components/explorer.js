...
const Explorer = () => {
  ...
  useEffect(() => {
    ...
    const contractAddresses = ['0x9c8ff314c9bc7f6e59a9d9225fb22946427edc03', '0x4b10701Bfd7BFEdc47d50562b76b436fbB5BdB3B', '0x73a8CE3f662a1e66563D05606BC5a8aB71a4D924', '0x643AfeE78bD8C693Be049AA40e337fD15CdFB61b',  '0x15a2d6c2b4b9903c27f50cb8b32160ab17f186e2', '0xD6c4cB5A3e040abC8bE977dC10B658Ec9072a1F4'];
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('IPFS fetch timed out')), 2000); 
    });

    const response = await Promise.race([fetch(ipfsUrl), timeoutPromise]);

    if (response.status === 504) {
      console.warn('IPFS fetch timed out for:', ipfsLink);
      return null; 
    }
